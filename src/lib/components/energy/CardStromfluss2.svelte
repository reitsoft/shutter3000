...
<script lang="ts">
    ...
    // 2. Gesamter Zufluss und Verbrauch im Gebäudenetz
    ...
    const sankeyDataV2: ChartData<'sankey', { from: string; to: string; flow: number }[]> = {
        datasets: [
            {
                data: [
                    { from: 'PV', to: 'Gebäudenetz', flow: PV_AN_GEBAEUDE },
                    { from: 'PV', to: 'Speicher', flow: BATTERIE_LADUNG },
                    { from: 'Speicher', to: 'Gebäudenetz', flow: BATTERIE_ENTLADUNG },
                    { from: 'Netzbezug', to: 'Gebäudenetz', flow: NETZBEZUG },
                    { from: 'Gebäudenetz', to: 'Haushalt', flow: HAUSHALT },
                    { from: 'Gebäudenetz', to: 'Heizung', flow: HEIZUNG },
                    { from: 'Gebäudenetz', to: 'Warmwasser', flow: WARMWASSER },
                ...
                colorFrom: (ctx) => {
                    ...
                    return '#3b82f6'; // Gebäudenetz → Verbraucher / Einspeisung
                },
                colorTo: (ctx) => {
                    const key = ctx.dataset.data[ctx.dataIndex].to;
                    if (key === 'Speicher') return '#22c55e';
                    if (key === 'Einspeisung') return '#64748b';
                    if (key === 'Gebäudenetz') return '#3b82f6';
